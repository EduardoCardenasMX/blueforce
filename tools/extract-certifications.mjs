...
const sourceFiles = {
  "ux-designer": "salesforce_platform_user_experience_designer_study_lab.html",
...
const metadata = {
  "ux-designer": {
    ...
    blueprint: [
      ...
      { label: "Salesforce Lightning Design System (SLDS)", weight: 21 },
  ...
  "business-analyst": {
    ...
    blueprint: [
      ...
      { label: "Development Support and User Acceptance", weight: 12 },
    ],
    examConfig: {
      officialQuestionCount: 60,
      officialDurationMinutes: 105,
...
function validateQuestion(certificationId, question, index) {
  const label = `${certificationId} question ${question.id || index + 1}`;
  if (!Number.isInteger(question.id)) throw new Error(`${label} is missing a numeric id`);
  if (!question.category) throw new Error(`${label} is missing category`);
  if (!question.question) throw new Error(`${label} is missing question text`);
  if (!Array.isArray(question.options) || question.options.length < 2) {
    throw new Error(`${label} must include at least two options`);
  }
  if (!Array.isArray(question.answers) || !question.answers.length) {
    throw new Error(`${label} must include answers[]`);
  }
  if (question.select !== question.answers.length) {
    throw new Error(`${label} select must match answers length`);
  }
  question.answers.forEach((answer) => {
    if (!Number.isInteger(answer) || answer < 0 || answer >= question.options.length) {
      throw new Error(`${label} has an invalid answer index: ${answer}`);
    }
  });
  if (!question.explanation) throw new Error(`${label} is missing explanation`);
  if (!question.tip) throw new Error(`${label} is missing tip`);
}
...
function normalizeQuestions(certificationId, questions) {
  return questions.map((question, index) => {
    ...
    const normalizedQuestion = {
      ...
      options: options.map((option) => normalizeOptionText(option)),
      answers,
      explanation: normalizeText(question.explanation),
      tip: normalizeText(question.tip),
    };
    validateQuestion(certificationId, normalizedQuestion, index);
